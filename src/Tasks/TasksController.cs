...
public class TasksController : ControllerBase
{
    ...
    public async Task<IActionResult> Post([FromServices] IConfiguration configuration, [FromServices] IEnumerable<ITask> tasks, [FromBody] JsonContent content, [FromRoute] string taskId)
    {
        ...
        if (accessToken == content.AccessToken && tasks.FirstOrDefault(t => t.GetType().Name.Equals(taskId.Replace(" ", ""), System.StringComparison.InvariantCultureIgnoreCase)) is ITask task)
        {
            await task.Run();
            return Ok();
        }
